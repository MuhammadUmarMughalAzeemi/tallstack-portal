<form wire:submit="submit" class="space-y-8">
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-4">
        <h3 class="text-base font-semibold text-indigo-400 border-b border-slate-800 pb-2">3. Bachelor's</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-xs text-slate-400 mb-1">Examination Passed (Bachelor's) <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">Expertise in Subjects <span class="text-rose-500">*</span></label>
                <input type="text" wire:model.blur="mbbsScienceSubjects" placeholder="e.g. Nephrology"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">Institution Type <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">University Name <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">Choose Year Of Passing <span class="text-rose-500">*</span></label>
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">CGPA (out of 4) / Obtained Marks <span class="text-rose-500">*</span></label>
                <input type="number" step="0.01" wire:model.blur="mbbsMarksObtained"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
            ...
            <div>
                <label class="block text-xs text-slate-400 mb-1">CGPA / Total Marks <span class="text-rose-500">*</span></label>
                <input type="number" step="0.01" wire:model.blur="mbbsTotalMarks"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
    ...
    <!-- M.Phil Section (Only for Ph.D applicants) -->
    <div class="bg-slate-900/80 border {{ $isPhd ? 'border-indigo-500/40' : 'border-slate-800' }} rounded-2xl p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
            <div class="flex items-center gap-2">
                <h3 class="text-base font-semibold text-indigo-400">4. M.Phil (Only for those applying for Ph.D)</h3>
                @if($isPhd)
                    <span class="text-[10px] font-bold uppercase tracking-widest bg-rose-500/10 text-rose-400 border border-rose-500/30 px-2 py-0.5 rounded-full">Required for Ph.D</span>
                @endif
            </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    Examination Passed (M.Phil) @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <select wire:model="mphilPassed"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilPassed') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                    <option value="">Select M.Phil Exam</option>
                    @foreach($mphilExams as $e)<option value="{{ $e->id }}">{{ $e->name }}</option>@endforeach
                </select>
                @error('mphilPassed') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    Expertise in Subjects @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <input type="text" wire:model.blur="mphilScienceSubjects" placeholder="e.g. Cardiology"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilScienceSubjects') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                @error('mphilScienceSubjects') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    Institution Type @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <select wire:model="mphilInstitutionType"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilInstitutionType') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                    <option value="">Select Type</option>
                    @foreach($institutionTypes as $it)<option value="{{ $it->id }}">{{ $it->name }}</option>@endforeach
                </select>
                @error('mphilInstitutionType') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    University Name @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <select wire:model="mphilBoard"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilBoard') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                    <option value="">Select University</option>
                    @foreach($boards as $b)<option value="{{ $b->id }}">{{ $b->name }}</option>@endforeach
                </select>
                @error('mphilBoard') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    Choose Year Of Passing @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <input type="text" wire:model.blur="mphilPassingYear" placeholder="2023"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilPassingYear') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                @error('mphilPassingYear') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    CGPA (out of 4) / Obtained Marks @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <input type="number" step="0.01" wire:model.blur="mphilMarksObtained" placeholder="3"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilMarksObtained') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                @error('mphilMarksObtained') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">
                    CGPA / Total Marks @if($isPhd) <span class="text-rose-500">*</span> @endif
                </label>
                <input type="number" step="0.01" wire:model.blur="mphilTotalMarks" placeholder="4"
                    class="w-full bg-slate-950 border rounded-lg px-3 py-2 text-xs text-slate-100 focus:outline-none transition-colors
                    {{ $errors->has('mphilTotalMarks') ? 'border-rose-500' : 'border-slate-800 focus:border-indigo-500' }}">
                @error('mphilTotalMarks') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
            </div>
        </div>
    </div>
    ...
    <div class="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
            <h3 class="text-base font-semibold text-indigo-400">Experience (Teaching / Practical / Reviewer / Editor of Journal)</h3>
            ...
            <div class="space-y-3">
                @foreach($experiences as $index => $exp)
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-3 p-3 bg-slate-950/60 rounded-xl border border-slate-800 items-end">
                        <div>
                            <label class="block text-[11px] text-slate-400 mb-1">Organization / Institute <span class="text-rose-500">*</span></label>
                            <input type="text" wire:model.blur="experiences.{{ $index }}.institute" placeholder="Organization name"
                                class="w-full bg-slate-900 border rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-indigo-500
                                {{ $errors->has('experiences.'.$index.'.institute') ? 'border-rose-500' : 'border-slate-800' }}">
                            @error('experiences.'.$index.'.institute') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-[11px] text-slate-400 mb-1">Designation / Position <span class="text-rose-500">*</span></label>
                            <input type="text" wire:model.blur="experiences.{{ $index }}.designation" placeholder="Your designation"
                                class="w-full bg-slate-900 border rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-indigo-500
                                {{ $errors->has('experiences.'.$index.'.designation') ? 'border-rose-500' : 'border-slate-800' }}">
                            @error('experiences.'.$index.'.designation') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-[11px] text-slate-400 mb-1">From Date <span class="text-rose-500">*</span></label>
                            <input type="date" wire:model="experiences.{{ $index }}.fromDate"
                                class="w-full bg-slate-900 border rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-indigo-500
                                {{ $errors->has('experiences.'.$index.'.fromDate') ? 'border-rose-500' : 'border-slate-800' }}">
                            @error('experiences.'.$index.'.fromDate') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-[11px] text-slate-400 mb-1">To Date <span class="text-rose-500">*</span></label>
                            <input type="date" wire:model="experiences.{{ $index }}.toDate"
                                class="w-full bg-slate-900 border rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-indigo-500
                                {{ $errors->has('experiences.'.$index.'.toDate') ? 'border-rose-500' : 'border-slate-800' }}">
                            @error('experiences.'.$index.'.toDate') <span class="text-xs text-rose-400 mt-1 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <button type="button" wire:click="removeExperience({{ $index }})"
                                class="w-full py-1.5 bg-rose-500/10 border border-rose-500/30 text-rose-400 rounded-lg text-xs font-medium hover:bg-rose-500/20 transition-colors">
                                Remove
                            </button>
                        </div>
                    </div>
                ...
                <button type="button" wire:click="addExperience"
                    class="px-4 py-2 bg-indigo-600/20 border border-indigo-500/40 text-indigo-300 rounded-xl text-xs font-semibold hover:bg-indigo-600/30 transition-colors">
    ...
    <div class="pt-6 flex justify-between gap-4 border-t border-slate-800">
        ...
        <button type="submit" wire:loading.attr="disabled" wire:target="submit"
            ...
            <span wire:loading.remove wire:target="submit">Save & Proceed to Preferences</span>
